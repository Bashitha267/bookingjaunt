                ...
                if ($request['request_type'] === 'delete') {
                    ...
                    if ($new_data) {
                        ...
                        $stmt->execute([
                            ...
                            // Payment Options
                            ($new_data['pay_cash'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['pay_cc'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['pay_debit'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['pay_online'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['pay_bank'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['pay_installments'] ?? '0') == '1' ? 1 : 0,
                            isset($new_data['refund_supported']) && $new_data['refund_supported'] !== '' ? (int)$new_data['refund_supported'] : null,
                            isset($new_data['advance_payment_required']) && $new_data['advance_payment_required'] !== '' ? (int)$new_data['advance_payment_required'] : null,
                            $new_data['payment_notes'] ?? null,
                            !empty($new_data['custom_payments']) ? json_encode(array_values(array_filter($new_data['custom_payments']))) : null,
                            // Food & Dining
                            ($new_data['food_breakfast_included'] ?? '0') == '1' ? 1 : 0,
                            $new_data['food_breakfast_type'] ?? null,
                            ($new_data['food_restaurant_available'] ?? '0') == '1' ? 1 : 0,
                            isset($new_data['food_restaurant_count']) && $new_data['food_restaurant_count'] !== '' ? (int)$new_data['food_restaurant_count'] : null,
                            ($new_data['food_room_service'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_room_service_247'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_vegetarian'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_vegan'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_halal'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_buffet'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_delivery_allowed'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_dietary_options'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_kitchen_in_room'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['food_minibar'] ?? '0') == '1' ? 1 : 0,
                            $new_data['food_notes'] ?? null,
                            !empty($new_data['custom_food']) ? json_encode(array_values(array_filter($new_data['custom_food']))) : null,
                            // Security
                            ($new_data['sec_staff_247'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_cctv'] ?? '0') == '1' ? 1 : 0,
                            $new_data['sec_cctv_coverage'] ?? null,
                            ($new_data['sec_smoke_detectors'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_fire_extinguishers'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_fire_alarm'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_emergency_exit_plan'] ?? '0') == '1' ? 1 : 0,
                            $new_data['sec_emergency_evac_instructions'] ?? null,
                            $new_data['sec_patrol_frequency'] ?? null,
                            ($new_data['sec_key_card_access'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_digital_lock'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_biometric_access'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_safe_box'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_luggage_storage'] ?? '0') == '1' ? 1 : 0,
                            $new_data['sec_parking_security'] ?? null,
                            ($new_data['sec_female_floor'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_panic_button'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_first_aid'] ?? '0') == '1' ? 1 : 0,
                            ($new_data['sec_medical_support'] ?? '0') == '1' ? 1 : 0,
                            $new_data['sec_hospital_distance'] ?? null,
                            $new_data['sec_notes'] ?? null,
                            !empty($new_data['custom_security']) ? json_encode(array_values(array_filter($new_data['custom_security']))) : null,
                            $property_id
                        ]);
