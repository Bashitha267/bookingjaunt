...
<head>
    ...
    <title>Register Your Vehicle - Bookingjaunt</title>
...
<body class="min-h-screen flex flex-col">
    ...
    <div
        ...
        <div class="min-w-[600px] md:min-w-0 max-w-[1600px] mx-auto px-6">
            <div class="flex justify-between relative">
                <?php
                $steps = ["Vehicle", "Capacity", "Pricing", "Driver", "Features", "Location", "Bank"];
    ...
    <main class="w-full max-w-[1600px] mx-auto pb-24 px-4 sm:px-8">
        <form id="property-form"
            ...
            <!-- Step 1: Business Type (Hidden for Vehicle Wizard) -->
            <div class="wizard-step active" data-step="1" style="display:none;">
                <input type="radio" name="business_type" value="vehicle" class="hidden" checked>
            </div>
            ...
            <div class="wizard-step" data-vstep="7" style="display:none;">
                ...
                <div class="space-y-8">
                    <div>
                        ...
                        <div class="upload-container relative group" id="vehicle-cover-upload">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                        ...
                        <div class="md:col-span-3">
                            ...
                            <div class="upload-container relative group" id="property-cover-upload">
                                <input type="file" accept="image/*" class="hidden file-input" data-type="cover">
                                <input type="hidden" name="cover_image">
                                <div
    ...
    <script>
        ...
        function changeStep(delta) {
            if (isVehicleMode() && (currentStep > 1 || delta > 0)) {
                ...
                if (currentStep === 1 && delta > 0) {
                    ...
                    // Do nothing to prevent going back to Step 1
                }
        ...
        function getCoverUploadId(businessType) {
            return businessType === 'vehicle' ? 'vehicle-cover-upload' : 'property-cover-upload';
        }
        ...
        function loadFormData() {
            if (editMode && editData) {
                ...
                const fields = [
                    ...
                    'vehicle_category','brand','model','manufactured_year','registration_number',
                    'chassis_number','engine_number','vehicle_color','fuel_type','transmission_type',
                    'vehicle_condition','seat_count','luggage_count','max_passengers','pricing_type',
                    'price_per_day','included_km_per_day','extra_km_price','price_per_km',
                    'hourly_price','weekly_price','monthly_price','discount_daily','discount_weekly',
                    'discount_monthly','discount_seasonal','coupon_support','driver_option',
                    'driver_name','driver_contact','driver_whatsapp','driver_nic','driver_license',
                ...
                if (editData.business_type) {
                    updateCategoryVisibility(editData.business_type);
                    updateVehicleSteps();
                }
                ...
                if (editData.cover_image) {
                    const coverId = getCoverUploadId(editData.business_type);
                    showPreview(coverId, editData.cover_image);
                    const coverBox = document.getElementById(coverId);
                    const inp = coverBox ? coverBox.querySelector('input[type="hidden"]') : null;
            ...
            if (savedData) {
                ...
                if (data.cover_image) {
                    const coverId = getCoverUploadId(data.business_type);
                    showPreview(coverId, data.cover_image);
                }
        ...
        propertyForm.onsubmit = async (e) => {
            ...
            const propertyIdInput = document.getElementById('form-property-id');
            if (actionInput) actionInput.value = editMode ? 'request_edit' : 'register_property';
            if (propertyIdInput) propertyIdInput.value = editMode ? propertyId : '';

            const formData = new FormData(propertyForm);
            if (editMode) {
                formData.set('action', 'request_edit');
                formData.set('property_id', propertyId);
            } else {
                formData.set('action', 'register_property');
            }
            ...
            try {
                ...
                if (contentType.includes('application/json')) {
                    ...
                    console.error('Non-JSON response from register.php:', text.slice(0, 500));
